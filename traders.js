(function() {
    ...
    var balperscrip = process.env.BPS || 120000;
    ...
    function findNEdge(respArr, n) {
        ...
        _selectedScrips.forEach(function(scrip) {
            sList += scrip.sym + ",";
            sListNotify += scrip.sym + "\n";
        });
        Worker.logme("Selected scrips : [" + sList + "]");
        Worker.notifyMe("`Selected scrips :\n\n" + sListNotify + "`");
            ...
            .then(function() {
                ...
                upstox.on("orderUpdate", function(message) {
                    ...
                    if (message.status == "trigger pending") {
                        ...
                        Worker.notifyMe("`Trigger|" + message.transaction_type + "|" + message.symbol+"`");
    ...
    function placeTargerOrder(pos) {
        ...
        upstox.placeOrder({
            ...
            .catch(function(error) {
                ...
                Worker.notifyMe("`Placing trgt order err..`");
            });
    }
    ...
    function getCurrentPos() {
        upstox.getPositions()
            .then(function(response) {
                ...
                Worker.logme("--------------------------------------------");
                Worker.notifyMe("`Unrealized MTM: " + mtm.toFixed(2)+"`");
            }).catch(function(error) {
                //done(error);
